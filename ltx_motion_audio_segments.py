...
def _trim_extra_video_frame(video):
    if not hasattr(video, "get_components"):
        return video
    ...
    # Если кадров больше одного, отрезаем последний нахуй
    if images is not None and images.shape[0] > 1:
        components.images = images[:-1]
        logging.warning(f"[LTX Motion] Dropped 1 frame for A/V sync. New frames count: {components.images.shape[0]}")
...
def _save_video_output(video, filename_prefix, prompt=None, extra_pnginfo=None):
    ...
    video = _trim_extra_video_frame(video) # <--- ДОБАВЬ ЭТУ СТРОКУ
...
class LTXMotionAudioSegmentLoop:
    ...
    def queue_next(
        ...
        video = _sanitize_video_images(video)
        video = _sanitize_video_audio(video)
        video = _trim_extra_video_frame(video) # <--- И ДОБАВЬ ЭТУ СТРОКУ СЮДА
